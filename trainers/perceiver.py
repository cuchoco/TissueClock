...
class PerceiverTrainer:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.output_dir = Path(cfg.output_dir) / f"tissue_perceiver_fold{cfg.fold}"
        ...
        self.latent_seq = model_params.get('latent_seq', 64)
        self.latent_dim = model_params.get('latent_dim', 512)
        self.tissue_embed = model_params.get('tissue_embed', False)
        self.perceiver_depth = model_params.get('perceiver_depth', 4)
        self.transformer_depth = model_params.get('transformer_depth', 2)
        self.xattn_heads = model_params.get('xattn_heads', 8)
        self.mhsa_heads = model_params.get('mhsa_heads', 8)
        self.share_xattn_start_layer = model_params.get('share_xattn_start_layer', 1)
        self.share_tf_start_layer = model_params.get('share_tf_start_layer', 0)
        self.mlp_mult = model_params.get('mlp_mult', 1)
        self.mlp_activation = model_params.get('mlp_activation', 'geglu')
    ...
    def _build_model(self):
        """Build model"""
        self.accelerator.print(f"Building TissuePerceiverMIL model...")
        self.accelerator.print(f"  in_dim: {self.in_dim}")
        self.accelerator.print(f"  latent_seq: {self.latent_seq}")
        self.accelerator.print(f"  latent_dim: {self.latent_dim}")
        self.accelerator.print(f"  tissue_embed: {self.tissue_embed}")
        self.accelerator.print(f"  perceiver_depth: {self.perceiver_depth}")
        self.accelerator.print(f"  transformer_depth: {self.transformer_depth}")
        self.accelerator.print(f"  xattn_heads: {self.xattn_heads}")
        self.accelerator.print(f"  mhsa_heads: {self.mhsa_heads}")
        
        self.model = TissuePerceiverMIL(
            ...
            share_xattn_start_layer=self.share_xattn_start_layer,
            share_tf_start_layer=self.share_tf_start_layer,
    ...
    def train(self):
        ...
        self.accelerator.print(f"Starting Perceiver MIL Training (Fold {self.fold})")
        self.accelerator.print("=" * 80)
...
def train(cfg):
    """Train function for Perceiver MIL model (compatible with TRAINERS registry)"""
    trainer = PerceiverTrainer(cfg)
    history = trainer.train()
    return history


@hydra.main(version_base=None, config_path="config", config_name="config_perceiver")
