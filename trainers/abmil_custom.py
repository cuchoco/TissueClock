...
from model.abmil_custom import TissueABMIL


class ABMILTrainer:
    """
    ABMIL (Attention-Based Multiple Instance Learning) Trainer
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.num_tissues = cfg.get('num_tissues', 50)
    ...
    def _build_model(self):
        """Build model"""
        self.accelerator.print(f"Building TissueABMIL model...")
        self.accelerator.print(f"  num_tissues: {self.num_tissues}")
        self.accelerator.print(f"  feature_dim: {self.feature_dim}")
        self.accelerator.print(f"  head_dim: {self.head_dim}")
        self.accelerator.print(f"  n_heads: {self.n_heads}")
        self.accelerator.print(f"  gated: {self.gated}")
        self.accelerator.print(f"  tissue_embed: {self.tissue_embed}")
        
        self.model = TissueABMIL(
    ...
    def train(self):
        ...
        self.accelerator.print(f"Starting ABMIL Training (Fold {self.fold})")
        self.accelerator.print("=" * 80)
...
def train(cfg):
    """Train function for ABMIL model (compatible with TRAINERS registry)"""
    trainer = ABMILTrainer(cfg)
    history = trainer.train()
    return history


@hydra.main(version_base=None, config_path="config", config_name="config_abmil")
